...
void read_matrix(char* filename)
{
    ...
    if(mtype==1)
    {
        for(int i=0;i<nnz;i++)
        {
            ...
            triplets[i].col--; // correction of column index from matrix file
        }
    }
...
void convertToELLPACK()
{
    ...
    printf("\nMax no. of non-zeroes values in a row: %d\n",max_nnz);
    ...
    // Convert the triplets to ELLPACK format
    for (int i = 0; i < nnz; i++)
    {
 ...
 * --------------------------
 *   Multiplies the triplet matrix with a vector by parallelization
 *   
 ...
 * ----------------------------------------
 *   Multiplies the triplet matrix with a vector in both serial and parallel methods
 *
 *   returns: None
 *   
 */
void ellpack_matrix_vector_multiply()
{
    ...
    */
    ...
    printf("\nComputation time for ELLPACK parallel implementation: %f seconds\n",comp_ptime);
    double pflops = (2*nnz)/comp_ptime;
    printf("FLOPS for ELLPACK parallel implementation: %.2f\n",pflops);
